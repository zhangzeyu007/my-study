 ...
 * @LastEditors: 张泽雨
 * @LastEditTime: 2024-03-24 22:05:41
...
这个执行顺序是有道理的,因为在渲染阶段,父组件需要先被创建,然后才能创建子组件。
在销毁阶段,则需要先销毁子组件,再销毁父组件。
理解父子组件生命周期钩子函数的执行顺序非常重要,因为它可以帮助你更好地管理组件的状态和行为。
比如,如果你需要在父组件挂载后获取数据并传递给子组件,那么你就需要在父组件的 mounted 钩子函数中进行这个操作。
...
渲染阶段:Vue 3 引入了全新的 setup 钩子函数,它会在 beforeMount 钩子函数之后、渲染函数之前执行,
并且在父子组件之间的执行顺序也保持不变。
新增的 setup 钩子函数用于替代 beforeCreate 和 created 钩子函数。
更新阶段:与 Vue 2 保持一致。
卸载阶段:beforeDestroy 钩子函数被重命名为 beforeUnmount,新增了 unmounted 钩子函数在组件卸载后执行。
总的来说,Vue 3 在生命周期钩子函数的执行顺序上与 Vue 2 保持了很大的一致性,只是进行了部分钩子函数的重命名和新增。
掌握 Vue 3中父子组件生命周期钩子函数的执行顺序,可以帮助你更好地管理组件的状态和行为,
编写出更加健壮和可维护的 Vue 应用程序。
*/
</script>
